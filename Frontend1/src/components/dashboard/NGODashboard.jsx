...
function NGODashboard() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await fetch('http://localhost:4000/api/service'); // Ensure this is the correct endpoint
        if (!response.ok) {
              ...
              {services.length === 0 ? (
                ...
                services.map((service) => (
                  <div key={service.service_id} className="bg-white p-4 rounded-lg shadow-lg">
                    ...
                    <p className="text-gray-500">Posted by: User {service.user_id}</p>
                    <p className="text-gray-500">Timestamp: {service.timestamp}</p> {/* No Date formatting here */}
                  </div>
                ))
              )}
            </div>
